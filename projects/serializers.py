...
class ProjectCommentSerializer(serializers.ModelSerializer):
    """
    Serializer for reading project comments
    """
    ...
    class Meta:
        ...
        read_only_fields = ['id', 'date_created', 'project_title', 'replies']
    
    def get_replies(self, obj):
        """Get nested replies for this comment"""
        if hasattr(obj, 'replies'):
            # Get direct replies only (avoid deep nesting in serialization)
...
class ProjectCommentCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_name(self, value):
        ...
        forbidden_words = ['spam', 'test123', 'admin', 'administrator']
    ...
    def validate_message(self, value):
        ...
        spam_indicators = ['viagra', 'casino', 'lottery', 'winner', 'free money']
        message_lower = clean_message.lower()
        for indicator in spam_indicators:
            if indicator in message_lower:
                raise serializers.ValidationError("Your message contains content that is not allowed.")
        ...
        if len(words) > 5:
            ...
            if len(unique_words) / len(words) < 0.3:  # Less than 30% unique words
                raise serializers.ValidationError("Message appears to be spam or repetitive.")
        
        return clean_message
    
    def validate_parent_comment(self, value):
        """Validate parent comment exists and belongs to the same project"""
        if value:
            try:
                parent = ProjectComment.objects.get(id=value)
    ...
    def validate(self, attrs):
        """Cross-field validation"""
        # Ensure parent comment belongs to the same project (if provided)
        if attrs.get('parent_comment'):
            # This will be validated in the view since we need the project context
            pass
...
class ProjectGalleryImageSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ProjectGalleryImage
        fields = ['id', 'project', 'image_url', 'alt_text', 'caption', 'order', 'date_created']
        read_only_fields = ['id', 'date_created']